...
class sigmav:
    def __init__(self, filepath, gas, dtype=torch.float64, device='cpu', epsilon=1e-40):
    ...
    def _uniform_interpolation(self):
        ...
        ln = np.linspace(lden.min(), lden.max(), 1000)
        lt = np.linspace(ltemp.min(), ltemp.max(),1000)
    ...
    def _interp_lrate(self, ly):
        ...
        """
        ly = ly.clamp(self.lTadf11[0] + 1e-12, self.lTadf11[-1] - 1e-12)
        idx = (torch.searchsorted(self.lTadf11, ly) - 1).clamp(0, self.lTadf11.size(0) - 2)
